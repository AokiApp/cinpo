...
public final class CinpoGradleSupport {
    ...
    public static Object targetApiJar(Project project) {
        ...
        try {
            manifest = ManifestLoader.load(project.file("manifest.yaml").toPath());
        } catch (IOException e) {
            throw new UncheckedIOException("Failed to read Java Card manifest for applet compilation", e);
        }
